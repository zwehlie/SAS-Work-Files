...
proc sort data= class.Donors1 nodupkey;
by ID;
run;
proc sort data= class.Diabetes nodupkey;
by ID;
run;
data merged_data;
    merge class.Donors1 (in=a drop=units) class.Diabetes (in=b);
    by id;
    if a and b;
run;
proc print data=merged_data;
    title "List of Donor Data";
run;
...
proc sort data=class.set1 nodupkey;
by ID;
run;
proc sort data=class.set2 nodupkey;
by ID;
run;
data one2one;
merge class.set1 (in=a) class.set2 (in=b);
by id;
missing_set1=ifn(a,0,1);
missing_set2=ifn(b,0,1);
if a or b;
run;
proc print data=one2one;
    title "Set3";
...
from one2one
where missing_set1 = 1 or missing_set2 = 1;
select count(*) as records_in_set2_not_in_set1
from one2one
where missing_set1 = 1;
quit;
*There are 12 records total in the dataset since the log mentions that there are 12 observations. 
There are 6 records missing from the dataset along with 6 being complete. There is 1 record in set 2 but 
not in set 1.;
